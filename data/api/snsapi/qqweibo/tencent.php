<?php
include_once(BASE_DATA_PATH.DS.'api'.DS.'snsapi'.DS.'qqweibo'.DS.'config.php');
/**
 * OAuth授权类
...
class OAuth
{
    ...
    {
        if (isset($_SESSION['qqweibo']['t_access_token'])) unset($_SESSION['qqweibo']['t_access_token']);
        if (isset($_SESSION['qqweibo']['t_expire_in'])) unset($_SESSION['qqweibo']['t_expire_in']);
        if (isset($_SESSION['qqweibo']['t_code'])) unset($_SESSION['qqweibo']['t_code']);
        if (isset($_SESSION['qqweibo']['t_openid'])) unset($_SESSION['qqweibo']['t_openid']);
        if (isset($_SESSION['qqweibo']['t_openkey'])) unset($_SESSION['qqweibo']['t_openkey']);
        if (isset($_SESSION['qqweibo']['t_oauth_version'])) unset($_SESSION['qqweibo']['t_oauth_version']);
    }
}
...
class Tencent
{
    ...
    {
        if (isset($_SESSION['qqweibo']['t_access_token'])) {//OAuth 2.0 方式
            //鉴权参数
            $params['access_token'] = $_SESSION['qqweibo']['t_access_token'];
            $params['oauth_consumer_key'] = OAuth::$client_id;
            $params['openid'] = $_SESSION['qqweibo']['t_openid'];
        ...
        } elseif (isset($_SESSION['qqweibo']['t_openid']) && isset($_SESSION['qqweibo']['t_openkey'])) {//openid & openkey方式
            $params['appid'] = OAuth::$client_id;
            $params['openid'] = $_SESSION['qqweibo']['t_openid'];
            $params['openkey'] = $_SESSION['qqweibo']['t_openkey'];
            $params['clientip'] = Common::getClientIp();
            $params['reqtime'] = time();
            $params['wbversion'] = '1';
