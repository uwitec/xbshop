...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="index.php?act=sns_strace&op=stracelist"><span><?php echo $lang['admin_snstrace_tracelist'];?></span></a></li>
      ...
      <tbody>
        <tr>
          ...
          <td>
              ...
              <option value="0" <?php echo $_GET['search_state'] == '1'?'selected':''; ?>><?php echo $lang['admin_snstrace_stateshow'];?></option>
              <option value="1" <?php echo $_GET['search_state'] == '0'?'selected':''; ?>><?php echo $lang['admin_snstrace_statehide'];?></option>
            </select>
           </td>
  ...
  <form method='post' id="form_comment">
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        <?php if(!empty($output['scomm_list']) && is_array($output['scomm_list'])){ ?>
        <?php foreach($output['scomm_list'] as $k => $v){?>
        <tr class="hover edit">
          <td class="w24"><input type="checkbox" name="sc_id[]" value="<?php echo $v['scomm_id'];?>" class="checkitem"></td>
          <td><?php echo parsesmiles($v['scomm_content']);?></td>
          <td class="w150 align-center"><?php echo $v['scomm_membername'];?></td>
          <td class="w150 align-center"><?php echo @date('Y-m-d H:i:s',$v['scomm_time']);?></td>
          <td class="w150 align-center"><?php echo $v['scomm_state']==0?$lang['admin_snstrace_statehide']:$lang['admin_snstrace_stateshow'];?></td>
          <td class="w150 align-center"><a href="javascript:void(0);" onclick="if(confirm('<?php echo $lang['nc_ensure_del'];?>')){location.href='index.php?act=sns_strace&op=scomm_del&sc_id=<?php echo $v['scomm_id'];?>';}else{return false;}"><?php echo $lang['nc_del'];?></a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
...
function submit_form(type){
	if(type=='del'){
		...
		$('#form_comment').attr('action','index.php?act=sns_strace&op=scomm_del');
	}else if(type=='hide'){
		$('#form_comment').attr('action','index.php?act=sns_strace&op=scomm_edit&type=hide');
	}else if(type=='show'){
		$('#form_comment').attr('action','index.php?act=sns_strace&op=scomm_edit&type=show');
	}
	$('#form_comment').submit();
}
</script>